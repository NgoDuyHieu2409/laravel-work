...
abstract class ApiController extends Controller
{
    ...
    /**
     * Returns a empty data success (200) JSON response.
     *
     * @param string $message
     * @return \Illuminate\Http\JsonResponse
     */
    public function responseEmpty($message = 'Success.')
    {
        return response()->json([
            'status' => 200,
            'message' => $message,
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getWorker($uid)
    {
        $worker = Worker::where('uid', $uid)->first();
        if (!$worker) {
            return $this->responseUnprocessable(trans('message.user_not_found'));
        }
        return  $worker;
    }
}
